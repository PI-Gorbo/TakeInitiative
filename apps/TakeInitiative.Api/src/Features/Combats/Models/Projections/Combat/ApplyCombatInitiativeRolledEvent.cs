...
{
    public async Task<Combat> Apply(CombatInitiativeRolledEvent @event, Combat Combat,
        IEvent<CombatInitiativeRolledEvent> eventDetails, IQuerySession session)
    {
        ...
        [
            new CombatInitiativeRolled
            {
                Rolls =
                [
                    ..@event.EvaluatedRolls.OrderByDescending(x => x.Value.Initiative.Value, new InitiativeComparer())
                        .Select(rollDto => new InitiativeRolledDto()
                        {
                            ...
                            {
                                UnevaluatedCharacterHealth.Roll roll => ((CharacterHealth.Fixed)rollDto.Value.Health)
                                    .DiceRoll,
                                null => null,
                                _ => null,
                            }
        ...
        {
            events.Add(new TurnStarted { CharacterId = newInitiativeList[0].Id, });
        }
        ...
        return Combat with
        {
            ...
            [
                .. Combat.History,
                new() { Events = [.. events], Executor = user!.Id, Timestamp = eventDetails.Timestamp }
            ]
        };
    }
}