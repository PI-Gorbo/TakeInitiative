...
public class RollStagedCharacterIntoInitiativeCommandHandler(IDocumentStore Store, IInitiativeRoller roller) : CommandHandler<RollStagedCharacterIntoInitiativeCommand, Result<Combat>>
{
    ...
    public override async Task<Result<Combat>> ExecuteAsync(RollStagedCharacterIntoInitiativeCommand command, CancellationToken ct = default)
    {
        return await Store.Try(
            async (session) =>
            {
                ...
                var computedInitiativeRolls = roller.ComputeRolls(combat.StagedList.Where(x => x.Id.In(command.CharacterIds)).ToList(), combat.InitiativeList);
